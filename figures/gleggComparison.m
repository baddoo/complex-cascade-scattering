...
C = [0.0001,0.01,.1,1];
nC = numel(C);
kLoop = linspace(0,30,nFreq+1); kLoop(1)=[];
lift = zeros(nC,nFreq);
myAModes = 35;
...
%%
ADData=struct('spacDim',    [vaneDistDim*cos(stagAngDim*pi/180),vaneDistDim*sin(stagAngDim*pi/180)],...%Blade Spacing [h,d]
...
AAData=struct( 'omegaDim', [],...
               'omega',    kLoop(l),...                      %Tangential Frequency
               'kx',       'gust',...
               'kxDim',    [],...
               'ky',       [],...                       %Normal frequency
               'kyDim',    [],...               
...
%%
lift(m,l) = 1i*kLoop(l).*D(-kLoop(l).*newADData.M^2./newADData.Beta.^2,data)./newAAData.Amp(2);
...
spoU = pi^2*Beta^2*kLoop(l).*abs(ZPa.*DLP).^2.*real(1./SQRTa)./se./abs(newAAData.Amp(2).^2);
spoD = pi^2*Beta^2*kLoop(l).*abs(ZMa.*DLM).^2.*real(1./SQRTa)./se./abs(newAAData.Amp(2).^2);
...
hold on
for j = 1:(nC-1)
plot(kLoop,abs(lift(j+1,:)),'LineWidth',2,'Color',c(j,:));
...
hold on
for j = 1:(nC-1)
plot(kLoop,real(firstDSMode(j+1,:)),'LineWidth',2,'Color',c(j,:))
end
axis([0,kLoop(end),0,.014])
hold off
xlabel('$\omega$')
ylabel('$W_-(0)$')
legend(legendInfo,'Interpreter','latex','Location','northeast')
...
hold on
for j = 1:(nC-1)
plot(kLoop,real(secondDSMode(j+1,:)),'LineWidth',2,'Color',c(j,:))
    legendInfo{j+1} = ['$C_{II} = ' num2str(C(j+1)) '$']; 
end
axis([0,kLoop(end),0,.014])
hold off
...
clear legendInfo
for j = 1:(nC-1)
    plot(kLoop,real(firstDSModeDel(j+1,:)),'LineWidth',2,'Color',c(j,:))
    hold on
...
clear legendInfo
for j = 1:(nC-1)
    plot(kLoop,real(secondDSModeDel(j+1,:)),'LineWidth',2,'Color',c(j,:))
    hold on
    legendInfo{j} = ['$C_{II} = ' num2str(C(j+1)) '$']; 
end
