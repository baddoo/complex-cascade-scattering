...
amodes=Modes.amodes;
%% Define duct mods
[TP,TM,asymP] = findDuctModes(ADData,AAData,Modes);
...
error = max(abs(Kminus(largeVal,Kargs).*Kplus(largeVal,Kargs)./K(largeVal,ADData,AAData)-1));
...
[X,Y]=meshgrid(xp,yp);
Z = X + 1i*Y;
PhasePlot(Z,K(Z,ADData,AAData)./(Kplus(Z,Kargs).*Kminus(Z,Kargs)),'d');
