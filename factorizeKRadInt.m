...
tic
argFunRad = @(thetVar) 1i*r.*exp(1i*thetVar).*argFun(r.*exp(1i*thetVar));

Nprad = (real(integral(argFunRad,0,2*pi,'ArrayValued',true)/(2i*pi)));
Nrad = round(Nprad);
Nrad3 = permute(1:Nrad,[1,3,2]);

polyFunRad = @(xVar) bsxfun(@power,xVar,Nrad3);
newFunRad = @(thetVar) polyFunRad(r.*exp(1i*thetVar)).*argFunRad(thetVar);
pRad = integral(newFunRad,0,2*pi,'ArrayValued',true)/(2i*pi);
eRad = ones(1,Nrad+1);

for l = 1 :Nrad
    j = (1:l).';
    eRad(l+1) = 1./l.*sum(permute((-1).^(j-1).'.*eRad(l-j+1),[1,3,2]).*pRad(:,:,(1:l)),3);
end

myCloseRootsRad = roots((-1).^(1:Nrad+1).*eRad);
myCloseRootsRadFin = zeros(1,numel(myCloseRootsRad));
for nL = 1:(Nrad)
myCloseRootsRadFin(nL) = findRoots2(s,d,sigma,mu,k,w,myCloseRootsRad(nL));
end
