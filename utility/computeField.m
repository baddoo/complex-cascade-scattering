...
phiUp = pi*1i*ARLP.*tFacLP*D13LP.';

% Calculate upper triangular field
utLoc = (X > Y*d/s & X<=d);
Xut = X(utLoc); Yut = Y(utLoc); % Upstream triangle
phiUt =  pi*1i*Ad(Xut,Yut,TPd,zeta(TPd),spac,Sigma).*tFacTP*B ...
       - pi*Ad(Xut,Yut,GM,zetaGM0,spac,Sigma)*w0/(2*pi)^2/regK(GM,ADData,AAData).*tFacGM ...
       - pi*Ad(Xut,Yut,TMd,zeta(TMd),spac,Sigma)./(TMd - PM).*tFacTM*(A + C) ...
       + pi*1i*AuR(Xut,Yut,LPa,ZPa,SQRT,spac).*tFacLP*D13LP.';
...
AGM0 = Ad(Xre,Yre,GM,zeta(GM),spac,Sigma) + Au(Xre,Yre,GM,zeta(GM),spac,Sigma);

phiRe =  pi*1i*ATPd.*tFacTP*B ...
       - pi*AGM0*w0/(2*pi)^2/regK(GM,ADData,AAData)*tFacGM ...
       - pi*ATMd./(TMd - PM).*tFacTM*(A + C);
...
phiDt =  pi*1i*Au(Xdt,Ydt,TPd,zeta(TPd),spac,Sigma).*tFacTP*B ...
       - pi*Au(Xdt,Ydt,GM,zetaGM0,spac,Sigma)*w0/(2*pi)^2/regK(GM,ADData,AAData)*tFacGM ...
       - pi*Au(Xdt,Ydt,TMd,zeta(TMd),spac,Sigma)./(TMd - PM).*tFacTM*(A + C) ...
       - pi*1i*AdR(Xdt,Ydt,LMa,ZMa,SQRT,spac).*tFacLM*D24LM.' ...
       + 1i*pi*P*Ad(Xdt,Ydt,PM,zetaPM0,spac,Sigma).*tFacPM;
...
APM0 = Ad(Xdn,Ydn,PM,zeta(PM),spac,Sigma) + Au(Xdn,Ydn,PM,zeta(PM),spac,Sigma);
phiDs = - pi*1i*ARLM.*tFacLM*D24LM.' ...
        + 1i*pi*P*APM0.*tFacPM;
