...
c = hsv(nAng);
drawArrow = @(x,y,varargin) quiver( x(1),y(1),x(2)-x(1),y(2)-y(1),0,'MaxHeadSize',5, varargin{:} );       

for l0 = 1:nAng
plot(TPfin(:,:,l0).','Color',c(l0,:),'LineWidth',1.5)
for j = [1:nDM,nDM:2*nDM]
length = sum(abs(diff(TPfin(j,:,l0),1,2))); % Calculate length of each trajectory
[~,loc] = min(abs(cumsum(abs(diff(TPfin(j,:,l0),1,2)))-length/2));
arPts = [TPfin(j,loc+5,l0),TPfin(j,loc+6,l0)]; arPts= [real(arPts);imag(arPts)];
arrow(arPts(:,1),arPts(:,2),.2,'FaceColor',c(l0,:),'EdgeColor',c(l0,:))
end
end
...
% clf
% for l0 = 1:nAng
% semilogx(abs(C1vec),abs(imag(TPfin(7,:,l0))).','Color',c(l0,:),'LineWidth',2)
% hold on
% semilogx(abs(C1vec),abs(imag(TPfin(2,:,l0))).','--','Color',c(l0,:),'LineWidth',2)
% 
% end
% 
% matlab2tikz([imageFolder,'im-part',num2str(m),'.tex'], 'height', '\fheight', 'width', '\fwidth','parseStrings',false,'extratikzpictureoptions','trim axis left, trim axis right',...
%                  'extraaxisoptions','ylabel shift = -.4cm');
% xlim([abs(C1vec(2)),abs(C1vec(end))])             
% xlabel('$\Im[\theta]$');%,'Interpreter','Latex')
% ylabel('$|C_{II}|$');%,'Interpreter','LaTeX')
% hold off
% end       
